...
if [ "$CI" = "true" ]; then
    ...
    echo "deb [signed-by=/etc/apt/keyrings/gemstone-packages-keyring.gpg] https://packages.t3gemstone.org/apt/t3-gem-o1 jammy main" | tee /etc/apt/sources.list.d/gemstone.list
else
